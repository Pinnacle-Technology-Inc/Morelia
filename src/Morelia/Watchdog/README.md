...
Maximum automatic rebuild attempts for one recovery incident. After the final
attempt cannot be verified, the stream is stopped and reported as
`needs_action` so the host can choose a stream restart, session restart,
start, or stop command. Defaults to `3`.
...
rebuilds the worker, or waits for a fresh post-restart heartbeat. A recovery
incident becomes `unhealthy` with action `needs_action` only after its
...
- recovery action details, including restart-attempt and post-restart
...
5. Verifies the new worker with a PING through the DataFlow queue. If PING
   times out, it waits for fresh replacement-worker data for
   `ceil(failure_threshold / 2)` watcher intervals.

The watchdog makes at most `max_auto_restart_attempts` automatic rebuilds for
one incident. If every attempt fails verification, it stops the worker and
marks the stream `needs_action`; use `stream_command()` to select the next
recovery action. A dead or missing worker begins recovery immediately, while a
live worker with stale or absent data still waits for `failure_threshold`
