...
class TestSpawnStop:
    def test_spawn_starts_host_and_persists_port_token(
        self, sup_app, session_id, auto_supervisor
    ):
        with sup_app.app_context():
            session = db.session.get(Session, session_id)
            port = auto_supervisor.spawn(session)

            # DB row updated.
            assert session.runtime_port == port
            assert session.runtime_port > 0
            assert session.runtime_token is not None
            assert len(session.runtime_token) > 0

            # Host is serving on that port.
            status = _get_status(port)
            assert status["dataflow_id"] == "df-sup-1"
            assert status["phase"] == "idle"
            ownership = db.session.scalars(db.select(RuntimeOwnership)).one()
            assert ownership.runtime_id == status["runtime_id"]
            assert ownership.pid == status["pid"]
            assert ownership.port == port
            assert ownership.state is RuntimeOwnershipState.RUNNING

            auto_supervisor.stop(session)

    def test_stop_clears_db_columns_and_terminates_process(
        self, sup_app, session_id, auto_supervisor
    ):
        with sup_app.app_context():
            session = db.session.get(Session, session_id)
            auto_supervisor.spawn(session)

            # Grab the process reference before stop() removes it.
            entry = auto_supervisor._children["df-sup-1"]
            proc = entry.proc
            ...
            assert session.runtime_token is None
            ownership = db.session.scalars(db.select(RuntimeOwnership)).one()
    ...
    def test_double_spawn_raises_already_running(
        self, sup_app, session_id, auto_supervisor
    ):
        with sup_app.app_context():
            session = db.session.get(Session, session_id)
    ...
    def test_stop_retry_after_proof_missing_raises_stop_proof_missing_again(
        self, sup_app, session_id, auto_supervisor
    ):
        """A retry against the SAME dataflow, after teardown already left
        ownership UNCERTAIN with reason stop_proof_missing, must keep
        surfacing StopProofMissing (with its "retry with force" guidance) —
        not degrade into the unrelated, unactionable RuntimeNotTracked."""
        with sup_app.app_context():
            session = db.session.get(Session, session_id)
            RuntimeOwnershipRepository().create_starting(
                runtime_id="rt-unreachable-2",
                session_id=session.id,
                dataflow_id="df-sup-1",
                manifest_hash="hash-unreachable-2",
                token=None,
            )
            ...
            with pytest.raises(StopProofMissing):
                auto_supervisor.stop(session)

            # First attempt already popped the child; a plain retry has no
            # in-memory entry left at all — this is exactly the scenario that
            # used to fall through to RuntimeNotTracked.
            assert "df-sup-1" not in auto_supervisor._children

            with pytest.raises(StopProofMissing) as exc_info:
                auto_supervisor.stop(session)
            assert exc_info.value.dataflow_id == "df-sup-1"
            assert exc_info.value.runtime_id == "rt-unreachable-2"
...
class TestStopAllShutdown:
    """`stop_all` (the daemon-shutdown teardown) must finish each session's
    lifecycle — not just kill the process — or the session lingers ACTIVE with
    its device still CLAIMED, blocking the next start with DeviceConfigNotFree.
    """

    def test_stop_all_completes_sessions_and_releases_devices(
        self, sup_app, auto_supervisor
    ):
        from app.domain.enums import DeviceClaimState
        from app.services import device_configs, manifests

        with sup_app.app_context():
            session = _create_session_config_shape_session(
                name="shutdown-me",
                dataflow_id="df-shutdown-1",
                hardware_id="SD001",
            )
            config_id = session.device_flows[0]["device_config_id"]
            device_configs.claim(config_id, session.id)

            manifest = manifests.resolve(session.id, dataflow_id="df-shutdown-1")
            auto_supervisor.spawn(session, manifest=manifest)
            assert session.runtime_port is not None

            summary = auto_supervisor.stop_all()
...
class TestReconcile:
    ...
    def test_reattach_to_live_host(self, sup_app, session_id, auto_supervisor):
        """reconcile() adopts a live host and does NOT start a second process."""
        with sup_app.app_context():
            session = db.session.get(Session, session_id)
            original_port = auto_supervisor.spawn(session)
            original_proc = auto_supervisor._children["df-sup-1"].proc

            # Simulate a daemon restart: fresh supervisor instance, same DB.
            fresh_sup = HostSupervisor()
            fresh_sup.reconcile([db.session.get(Session, session_id)])

            entry = fresh_sup._children.get("df-sup-1")
            assert entry is not None, "reconcile must register the live host"
            assert entry.port == original_port, "must reattach to the same port"
            ownership = db.session.scalars(db.select(RuntimeOwnership)).one()
            assert ownership.state is RuntimeOwnershipState.ADOPTED
            assert ownership.adopted_at is not None

            # The original process must still be alive — no double-spawn.
            assert original_proc.poll() is None, "reconcile must not kill the existing host"

            # Clean up via the supervisor that owns the process handle.
            auto_supervisor.stop(session)

    def test_reconcile_marks_runtime_id_mismatch_uncertain(
        self, sup_app, session_id, auto_supervisor
    ):
        with sup_app.app_context():
            session = db.session.get(Session, session_id)
            auto_supervisor.spawn(session)
            real_runtime_id = auto_supervisor._children["df-sup-1"].runtime_id
            ownership = db.session.scalars(db.select(RuntimeOwnership)).one()
            ownership.runtime_id = "wrong-runtime-id"
            db.session.commit()
            ...
            try:
                ...
                assert "df-sup-1" not in fresh_sup._children
                assert ownership.state is RuntimeOwnershipState.UNCERTAIN
                assert ownership.details["status"]["runtime_id"] != "wrong-runtime-id"
            finally:
                ownership.runtime_id = real_runtime_id
                db.session.commit()
                auto_supervisor.stop(session)

    def test_stale_port_triggers_respawn(self, sup_app, session_id, auto_supervisor):
        """reconcile() spawns fresh when the recorded port has no live host."""
        with sup_app.app_context():
            session = db.session.get(Session, session_id)

            # Grab a closed port the OS briefly owned but is now dead.
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.bind(("127.0.0.1", 0))
                dead_port = s.getsockname()[1]
            # Nothing listening on dead_port from here on.
            ...
            try:
                ...
                entry = fresh_sup._children.get("df-sup-1")
                assert entry is not None, "reconcile must spawn a replacement"
                assert entry.port != dead_port, "must have a new port (fresh spawn)"
                assert entry.proc is not None
                assert entry.proc.poll() is None
            finally:
                db.session.expire_all()
                fresh_sup.stop(db.session.get(Session, session_id))
...
def test_quiesce_stops_poller_and_cancels_recovery_timers():
    supervisor = HostSupervisor()
    poller_stops = []
    supervisor._event_poller.stop = lambda: poller_stops.append(True)
    ...
    timer = FakeTimer()
    supervisor._recovery_timers["df-restart"] = timer

    result = supervisor.quiesce()

    assert result == {"tracked_runtime_count": 0}
    assert poller_stops == [True]
    assert timer.cancelled is True
    assert timer.joined is True
    assert supervisor._recovery_timers == {}

    def test_stale_port_with_session_config_shape_resolves_manifest_before_respawn(
        self, sup_app
    ):
        """Replacement spawn uses device_config_id session flows via the manifest resolver."""
        with sup_app.app_context():
            session = _create_session_config_shape_session(
                name="raw-stale-port",
                dataflow_id="df-raw-stale-port",
                hardware_id="RT002",
            )
            ...
            try:
                ...
                entry = fresh_sup._children.get("df-raw-stale-port")
                assert entry is not None
                assert entry.port != dead_port
                assert entry.proc is not None
                assert entry.proc.poll() is None
            finally:
                db.session.expire_all()
                fresh_sup.stop(db.session.get(Session, session.id))

    def test_already_tracked_dataflow_skipped(self, sup_app, session_id, auto_supervisor):
        """reconcile() is a no-op for dataflows already in the in-memory registry."""
        with sup_app.app_context():
            session = db.session.get(Session, session_id)
            auto_supervisor.spawn(session)
            original_entry = auto_supervisor._children["df-sup-1"]

            # reconcile() on the SAME supervisor — must not re-spawn.
            auto_supervisor.reconcile([db.session.get(Session, session_id)])

            assert auto_supervisor._children["df-sup-1"] is original_entry
            assert auto_supervisor._children["df-sup-1"].port == original_entry.port
