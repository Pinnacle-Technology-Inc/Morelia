...
def test_device_list_renders_scan_rows_with_pool_status(monkeypatch):
    ...
    assert fake.calls == [("GET", "/api/v1/devices/pool", None)]
    assert result.output == (
        "id\ttype\tport\thardware_id\tnickname\tcolor\tavailability\tstatus\towner\n"
        "1\tpod8206hr\tCOM3\tA1B2C\tTom\t#112233\tavailable\tfree\t-\n"
        "-\tpod8206hr\tCOM4\tB2C3D\t-\t-\tavailable\tunconfigured\t-\n"
    )


def test_device_config_posts_direct_config(monkeypatch):
    saved = {
        "id": 7,
        "type": "pod8206hr",
        "hardware_id": "A1B2C",
    ...
    fake = FakeDaemonClient(posts=[saved])
    _use_fake_client(monkeypatch, fake)
...
def test_device_config_resolves_port_and_accepts_short_type(monkeypatch):
    saved = {
        "id": 7,
        "type": "pod8206hr",
        "hardware_id": "A1B2C",
...
def test_device_config_prompts_for_parameters_when_omitted(monkeypatch):
    saved = {
        "id": 7,
        "type": "pod8206hr",
        "hardware_id": "A1B2C",
...
def test_device_config_posts_template_config(monkeypatch):
    saved = {"id": 8, "source_template": "pod-high"}
    fake = FakeDaemonClient(posts=[saved])
    _use_fake_client(monkeypatch, fake)
    ...
    assert fake.calls == [
        (
            "POST",
            "/api/v1/device-configs/from-template",
            {
...
def test_device_template_list_prints_daemon_templates(app, monkeypatch, tmp_path):
    fake = FakeDaemonClient(
        gets=[
            [
                {
    ...
    reference = (tmp_path / "device-templates").resolve().as_posix()
    assert result.output == (
        "source\tid\tname\ttype\tcontent_hash\treference\n"
        f"stored\t1\tpod-high\tpod8206hr\tabc123\t{reference}\n"
        f"stored\t2\tpod-low\tpod8206hr\tdef456\t{reference}\n"
    )
...
def test_device_template_list_does_not_register_local_templates(app, monkeypatch, tmp_path):
    fake = FakeDaemonClient(
        gets=[
            [
                {
        ...
        posts=[
            {
                ...
                "file_path": "device-templates/portable-template.toml",
            }
        ],
    )
    _use_fake_client(monkeypatch, fake)
    ...
    assert result.exit_code == 0, result.output
    reference = library_dir.resolve().as_posix()
    assert result.output == (
        "source\tid\tname\ttype\tcontent_hash\treference\n"
        f"stored\t1\tpod-high\tpod8206hr\tabc123\t{reference}\n"
    )
...
def test_device_template_list_shows_duplicate_content_hash_rows_by_default(app, monkeypatch, tmp_path):
    fake = FakeDaemonClient(
        gets=[
            [
                {
    ...
    assert result.output == (
        "source\tid\tname\ttype\tcontent_hash\treference\n"
        f"stored\t1\tpod-high\tpod8206hr\tabc123\t{reference}\n"
        f"stored\t2\tpod-high-copy\tpod8206hr\tabc123\t{reference}\n"
    )


def test_device_template_list_unique_collapses_duplicate_content_hash_rows(app, monkeypatch, tmp_path):
    fake = FakeDaemonClient(
        gets=[
            [
                {
    ...
    result = CliRunner().invoke(pinnacle, ["device", "template", "list", "--unique"])
...
def test_device_template_list_skips_stored_templates_own_backing_file(app, monkeypatch, tmp_path):
    """Scanning the library dir rediscovers a stored template's own mirrored
    file; that shouldn't render as a second, identical row."""
    artifact = {
        "name": "pod-high",
        "type": "pod8206hr",
        "parameters": {"preamp_gain": 10},
    ...
    library_dir = tmp_path / "device-templates"
    _use_template_library_dir(monkeypatch, library_dir)
    local_path = library_dir / "pod-high.toml"
    local_path.write_text(
        "\n".join(['name = "pod-high"', 'type = "pod8206hr"', "", "[parameters]", "preamp_gain = 10"]),
        encoding="utf-8",
    )
    ...
    assert result.output == (
        "source\tid\tname\ttype\tcontent_hash\treference\n"
        f"stored\t1\tpod-high\tpod8206hr\t{device_template_content_hash(artifact)}\t{reference}\n"
    )


def test_device_template_list_shows_local_duplicate_without_reposting(app, monkeypatch, tmp_path):
    artifact = {
        "name": "portable-template",
        "type": "pod8206hr",
        "parameters": {"preamp_gain": 10},
    ...
    local_path = library_dir / "portable-template.toml"
    local_path.write_text(
        "\n".join(['name = "portable-template"', 'type = "pod8206hr"', "", "[parameters]", "preamp_gain = 10"]),
        encoding="utf-8",
    )
...
def test_device_template_list_unique_hides_local_duplicate(app, monkeypatch, tmp_path):
    artifact = {
        "name": "portable-template",
        "type": "pod8206hr",
        "parameters": {"preamp_gain": 10},
    ...
    result = CliRunner().invoke(pinnacle, ["device", "template", "list", "-u"])
