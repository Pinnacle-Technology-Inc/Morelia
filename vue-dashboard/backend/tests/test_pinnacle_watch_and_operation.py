...
def test_session_watch_renders_streamed_sse_events_in_order(monkeypatch):
    fake = FakeDaemonClient(
        stream_lines=[
            ...
            'data: {"phase":"starting","sequence":1}',
            "",
            "id: 2",
            "event: runtime.report",
            'data: {"phase":"running","sequence":2}',
            "",
        ]
    )
    ...
    assert fake.calls == [("STREAM", "/api/v1/sessions/12/events|text/event-stream", None)]
...
def test_operation_list_prints_daemon_operations_table(monkeypatch):
    fake = FakeDaemonClient(
        gets=[
            [
                {
                    ...
                    "resolved_at": None,
                    "created_at": "2026-07-07T14:31:07",
                }
            ]
        ]
    )
    ...
    assert result.exit_code == 0, result.output
    assert fake.calls == [
        ("GET", "/api/v1/operations/?state=uncertain&session=12&dataflow=df-1", None)
    ]
    assert "operation_id\tstate\tcommand\tsession_id\tdataflow_id" in result.output
    assert "op-1\tuncertain\tstart\t12\tdf-1" in result.output
    assert "runtime_identity_mismatch" in result.output
