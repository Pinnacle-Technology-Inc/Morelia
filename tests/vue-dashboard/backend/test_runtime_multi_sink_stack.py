...
def test_all_selected_sinks_attach_to_one_source_in_manifest_order(tmp_path):
    ...
    network = runtime._flowgraph._network
    assert len(network) == 1
    pod, sinks = network[0]
    assert isinstance(pod, MockPod8206HR)
...
def test_runtime_context_injects_picklable_worker_boundary_seams(tmp_path):
    ...
    ctx = runtime._runtime_context(ManagedCsvSink, device_flow, DeviceType.POD8206HR, pod=object())
...
def test_degraded_state_maps_to_degraded_health_and_bounds_the_message(tmp_path):
    ...
    runtime._flowgraph._on_sink_error(
        {
            "source_id": "pod8206hr",
